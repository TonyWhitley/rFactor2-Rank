...
import os
import sys
# Long entries in the cch file spoil the error message appearance
maxErrorLineLength = 60
...
class _playerTrackStat:
    ...
    def __init__(
        ...
        try:
            ...
            # List lines from start of this block up (not including) to start
            # of next
            for line in self.lines:
                if len(line) > maxErrorLineLength:
                    # Long entries in the cch file spoil the error message
                    # appearance, trim them
...
class cchFile:
    ...
    def __init__(self, cchFileText, cchFileName):
        ...
        self.recordResults = [
            ('all the [CAREER] stuff which we do not care about', 'This is not used')]
        # -1 to lose the blank line at the end
        self._records = [self.recordsText[0].split('\n')[:-1]]
        # Lines in Header plus [CAREER] record
        lineNumberOfThisRecord = len(self._records[0])
        ...
        for record in range(1, len(self.recordsText)):
            ...
            self.recordResults.append(
                (playerTrackStat_o.parsed, playerTrackStat_o.error))
    ...
    def get_errors(self):
        ...
        _errors = []
        for result in range(1, len(self.recordResults)):
            if self.recordResults[result][0] is None:
